...
def tokenize_and_align_labels_with_overflow(
    examples,
    tokenizer,
    text_column_name,
    label_column_name,
    ...
    """
    Tokenize with sliding window for long documents and align NER labels.
    Handles overflows via `overflow_to_sample_mapping` so no index errors.
    """
    print("*** Tokenize and Align Labels (with overflow) ***")
    ...
    # Map from each new chunk back to its original sample
    sample_mapping = tokenized_inputs["overflow_to_sample_mapping"]

    labels = []
    word_ids_list = []

    for i in range(len(tokenized_inputs["input_ids"])):  # loop over *chunks*
        word_ids = tokenized_inputs.word_ids(batch_index=i)
        word_ids_list.append(word_ids)

        # which original document does this chunk come from?
        orig_idx = sample_mapping[i]
        orig_labels = examples[label_column_name][orig_idx]
        if isinstance(orig_labels, str):
            from ast import literal_eval
            try:
                orig_labels = literal_eval(orig_labels)
            except Exception:
        ...
        for word_idx in word_ids:
            if word_idx is None:
                ...
                label_ids.append(label_to_id[orig_labels[word_idx]])
            else:
                # continuation subword
                label_ids.append(
                    label_to_id[orig_labels[word_idx]] if label_all_tokens else -100
                )
