...
The processed clinical drug–disease dataset generated by this pipeline is publicly available on Zenodo:  
https://zenodo.org/records/19351891  

The release includes:

- **Filtered AACT snapshot**  
  (`raw_aact/mv_interventional_drug_studies_20260302.csv`)  
  A tabular extract of interventional, drug-related studies from ClinicalTrials.gov (AACT, snapshot: 2025-12-01).  
  It contains one row per trial (`nct_id`) with study metadata, aggregated intervention names and types, and aggregated condition names.  
  This file serves as the structured input for entity extraction and linking.

- **Ontology-linked drug–disease entities**  
  (`linked_to_ontologies/entities_drug_disease_clin.csv`)  
  The final processed dataset with normalized drug and disease annotations.  
  Disease entities are mapped to **MONDO**, and drug entities to **UMLS**.  
  The file includes cleaned entity strings, ontology identifiers, normalized labels, and mapped parent concepts, enabling consistent aggregation and downstream analysis.
