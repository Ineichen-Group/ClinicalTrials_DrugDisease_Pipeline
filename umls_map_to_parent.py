...
def save_stats(mapped_to_parent_clinical, save_folder_path="./data/umls/"):
...
def get_all_umls_ids_from_df(df: pd.DataFrame, id_column: str = "drug_umls_termid") -> Set[str]:
    ...
    for cell in df[id_column].fillna("").astype(str):
        for tid in cell.split("|"):
            tid = tid.strip()
            if tid and tid != "-1":
                all_umls_ids.add(tid)
...
def main(args):
    ...
    print(f"Total unique UMLS IDs in clinical data: {len(all_umls_ids_clinical)}")
    print(f"Total unique UMLS IDs in preclinical data: {len(all_umls_ids_preclinical)}")
    print(f"Total unique UMLS IDs across both datasets: {len(all_umls_ids)}")
